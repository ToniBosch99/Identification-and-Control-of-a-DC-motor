...
float Nu = 1.05720;
float Nx[2] = {1.0, -2.6747};
float Kd[2] = {-1.2702, -0.1387}; //{2.0001, 0.1268};//{0.7647, 0.0671};// 
float L[2] = {-0.2995, 3.025}; //{0.9, -5.7195}; //{0.7493, -5.5762};// 
float Gamma[2] = {0.1913, 2.8926};
float Phi[4] = {0.90840, 0.0414, -5.7195, 0.0051};

// VARIABLES AUXILIARS
float r = 0.0;
float u_eq = 0.0;
float x_eq1 = 0.0; float x_eq2 = 0.0;
float x_h1 = 0.0; float x_h2 = 0.0;
float e_c1 = 0.0; float e_c2 = 0.0;
float y_h = 0.0; // estat predit
float x_h[2] = {0.0, 0.0}; //estats de l'observador
float x_h_s[2] = {0.0, 0.0};  
float eo = 0.0;
...
if ((micros() - t_consigna) > 1E6){
    ...
    if (r == 100.0) r = 130.0; else r = 100.0;
    // Update
    t_consigna = micros();
 }
  ...
  u = u_eq + (Kd[0]*e_c1 + Kd[1]*e_c2); /*RPM*/
  ...
  x_h[0] = L[0]*eo + Gamma[0]*u + Phi[0]*x_h[0] + Phi[1]*x_h[1];
  x_h[1] = L[1]*eo + Gamma[1]*u + Phi[2]*x_h[0] + Phi[3]*x_h[1];
